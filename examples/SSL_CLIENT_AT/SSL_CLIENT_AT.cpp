...
{
    send_at("AT+CGREG");
    send_at("AT+COPS?"); // Sustituye +123456789 con el número de destino
    send_at("AT+CPSI?");
    send_at("AT+CGDCONT?"); // Sustituye +123456789 con el número de destino
    send_at("AT+CGDCONT=cid,\"IP\",\"claro\"");
    send_at("AT+CGACT=?");
    send_at("AT+CCHSTART");
    init_ssl();
}
